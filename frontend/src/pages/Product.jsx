...
const Product = () => {
  ...
  const { products, currency, addToCart } = useContext(ShoopContext)
          ...
          <div className='flex flex-col gap-4 my-8'>
            ...
            <div className='flex gap-2'>
              {productData.sizes.map((item, index) => (
                <button onClick={() => setSize(item)} className={` py-2 px-4 bg-gray-100 cursor-pointer hover:bg-gray-200 border border-gray-100 ${item === size ? 'border-orange-500' : ''}`} key={index}>{item}</button>
              ))}
            </div>
          </div>
